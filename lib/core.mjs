...
function inferRiskLevel(task) {
  ...
  if (l3Hints.some((hint) => text.includes(hint))) return 'L3';
  const l2Hints = [
    'implement',
    'fix',
    'change',
  ...
  if (l2Hints.some((hint) => text.includes(hint))) return 'L2';
  return 'L1';
}
...
function buildTaskContract(queue, task, options = {}) {
  const riskLevel = options.riskLevel ?? inferRiskLevel(task);
  return {
    version: 1,
    task_id: task.id,
    queue,
    ...
    requires_human_gate: riskLevel !== 'L1',
...
function buildAcceptancePlan(contract) {
...
export async function writeAcceptancePlan(root, queue, task, taskContract) {
  const dir = taskRuntimeDirFor(root, queue, task.id);
  const file = path.join(dir, 'acceptance_plan.json');
  const plan = buildAcceptancePlan(taskContract.contract);
...
function buildDevPlan(contract, acceptancePlan) {
  ...
  return {
    ...
    risk_level: contract.risk_level,
    requires_human_gate: contract.requires_human_gate,
    approach: 'Read the task contract and acceptance plan, make the smallest useful change, and record checkpoint evidence before claiming completion.',
...
function buildCheckpointReview(contract, acceptancePlan, checkpoint) {
  const status = checkpointReviewStatus(checkpoint);
  const failed = [];
  const passed = [];
  const blocked = [];
...
function revisionActionFromReview(review) {
  ...
  for (const item of review.blocked ?? []) {
    actions.push({
      ...
      suggested_fix: 'Resolve this blocker or request the required human/external input before the next checkpoint.'
    });
  }
...
async function assessRevisionPolicy(root, queue, task, revisionRequest, policy, options = {}) {
  ...
  if (currentSignature && policy.sameFailureThreshold) {
    const lineage = await queueLineage(root, queue, task.id);
    let contiguous = 1;
    const previousAttempts = lineage.attempts
...
export async function queueRevisionNext(root, queue, taskId, options = {}) {
  ...
  const revisionGuard = await assessRevisionPolicy(root, normalized, task, revisionRequest, revisionPolicy, options);
  if (!revisionGuard.allowed) {
    throw new Error(`Revision guard blocked next round for ${task.id}: ${revisionGuard.reasons.join('; ')}`);
  }
  const title = options.title ?? `${task.title} revision ${revisionRequest.next_checkpoint?.suggested_id ?? 'next'}`;
  const body = options.task ?? renderRevisionTaskBody(task, run, revisionRequest);
...
export async function codeTaskRun(root, queue, options = {}) {
  ...
  if (errors.length === 0 && verifyCommands.length > 0) {
    ...
    finalVerification.commands = await runVerifyCommands(verifyCommands, root, options.timeoutMs ?? options.config?.timeoutMs);
  ...
  try {
    ...
    try {
      ...
      if (preflight && preflight.exitCode !== 0) {
        ...
        const runContext = { env: taskPlanningEnv(root, taskContract, acceptancePlan, devPlan) };
