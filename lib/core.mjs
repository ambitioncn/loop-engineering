...
export async function codePatchVerify(root, options = {}) {
  ...
  const patchFile = path.resolve(root, safeRelativePath(options.patch, 'patch file'));
  ...
  const diffFiles = diffFilesFromPatch(patch);
  if (!patch.trim()) {
    return {
      patchFile: path.relative(root, patchFile),
  ...
  try {
    ...
    const applyCheck = await runCommand(`git apply --check --binary ${shellQuote(tempPatch)}`, {
      cwd: root,
