...
export async function ensureQueueDirs(root, queue) {
  ...
  await Promise.all(['inbox', 'active', 'done', 'failed', 'runs']
...
export async function queueStatus(root, queue) {
  ...
  return {
    ...
    active: (await listJson(queueSubdirFor(root, queue, 'active'))).length,
...
export async function runQueueOnce(root, options) {
  ...
  try {
    ...
    if (preflight && preflight.exitCode !== 0) {
      ...
      dispatch = await runCommand(options.dispatcher, {
        cwd: root,
        env: buildQueueEnv(root, queue, task, activeFile, runId),
        timeoutMs
      });
